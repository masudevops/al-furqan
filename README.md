...
- Public Sunnah browsing remains disabled while the owner awaits a submitted Sunnah.com API-access request. sunnah.now was declined for single-collection coverage; UmmahAPI Hadith remains only a fallback candidate because its live catalog identifies fawazahmed0/jsDelivr as its upstream source and tested grades do not name their grading authority.
...
- Do not set `SUNNAH_NOW_ENABLED=true`. A scholarly vetted multi-collection Hadith provider with traceable references and grading provenance is still required.
