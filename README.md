...
- Sunnah browsing uses UmmahAPI as an interim provider while the owner awaits Sunnah.com API access. It supports dynamically discovered major collections, Arabic/English reading, provider full-text search, pagination, provider-supplied grades, and local bookmarks. The UI discloses the fawazahmed0/jsDelivr upstream and never invents a grading authority or missing reference metadata.
- The landing page includes a deterministic Verse of the Day from Quran.Foundation and Hadith of the Day from the enabled UmmahAPI catalog. Both change once per UTC date and fail independently without substitute religious content.
- A 99 Names of Allah section remains blocked until the source supplies a traceable edition, scholarly basis, and translation/meaning provenance; UmmahAPI currently supplies none of those fields.
...
- `HADITH_ENABLED=false` disables the interim provider immediately. `UMMAH_API_KEY` is optional for higher limits and must remain server-only. Sunnah.com remains the preferred future source if access is approved.
