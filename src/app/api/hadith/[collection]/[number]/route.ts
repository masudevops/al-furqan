...
export async function GET(_request: Request, { params }: { params: { collection: string; number: string } }) {
  ...
  try {
    const item = await sunnahNowHadithAdapter.one(params.collection, number);
    return item ? publicContentJson({ error: null, item }) : publicContentJson({ error: "Hadith not found.", item: null }, 404);
  } catch (error) {
    console.error("Sunnah.now Hadith detail request failed", { collection: params.collection, message: error instanceof Error ? error.message : "Unknown error", number });
