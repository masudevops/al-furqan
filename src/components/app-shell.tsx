...
type QuranBrowseMode = "surah" | "juz" | "revelation";
type SearchPayload = {
  error: string | null;
...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  const [quranBrowseMode, setQuranBrowseMode] = useState<QuranBrowseMode>("surah");
  const [quranBrowseAscending, setQuranBrowseAscending] = useState(true);
  ...
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get("settings") === "open") setSettingsOpen(true);
  }, [pathname]);

  useEffect(() => {
    if (!settingsOpen) return;
    if (!restoreFocusRef.current) restoreFocusRef.current = document.activeElement instanceof HTMLElement && document.activeElement !== document.body ? document.activeElement : settingsTriggerRef.current;
    const dialog = settingsDialogRef.current;
    const focusable = () => Array.from(dialog?.querySelectorAll<HTMLElement>('button:not([disabled]), select:not([disabled]), input:not([disabled]), [href]') ?? []);
    focusable()[0]?.focus();
    ...
    return () => { document.removeEventListener("keydown", onKeyDown); restoreFocusRef.current?.focus(); restoreFocusRef.current = null; };
  }, [settingsOpen]);

  const copyVerseLink = async (verseKey:string,chapter:number|null,verse:number|null,details:HTMLDetailsElement|null) => {
    try {
      await navigator.clipboard.writeText(`${location.origin}/quran/${chapter}/${verse}`);
      setCopiedVerseKey(verseKey);
      if(details)details.open=false;
      window.setTimeout(()=>setCopiedVerseKey(current=>current===verseKey?null:current),1800);
    } catch { setCopiedVerseKey(null); }
  };

  const shareVerseLink = async (verseKey:string,chapter:number|null,verse:number|null,details:HTMLDetailsElement|null) => {
    const url = `${location.origin}/quran/${chapter}/${verse}`;
    try {
      if (navigator.share) {
        await navigator.share({ title: `Quran ${verseKey}`, text: `Quran ${verseKey}`, url });
        if(details)details.open=false;
        return;
      }
      await copyVerseLink(verseKey,chapter,verse,details);
    } catch (error) {
  ...
  const translationGroups = useMemo(() => {
    const groups = new Map<string, TranslationResource[]>();
    for (const item of translations?.items ?? []) {
      const language = item.languageName?.trim() || "Other languages";
      groups.set(language, [...(groups.get(language) ?? []), item]);
    }
    return Array.from(groups.entries()).sort(([a], [b]) => a === "English" ? -1 : b === "English" ? 1 : a.localeCompare(b));
  }, [translations]);
  ...
  useEffect(() => {
    if (!playingVerseKey || !followRecitation) return;
    const activeVerse = document.querySelector<HTMLElement>(`[data-verse-key="${playingVerseKey}"]`);
    if (!activeVerse) return;

    const frame = window.requestAnimationFrame(() => {
      const bounds = activeVerse.getBoundingClientRect();
      const topBoundary = window.innerWidth <= 900 ? 136 : 92;
      const bottomBoundary = window.innerHeight - (window.innerWidth <= 900 ? 168 : 104);
      if (bounds.top >= topBoundary && bounds.bottom <= bottomBoundary) return;
      const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
      activeVerse.scrollIntoView({ behavior: reducedMotion ? "auto" : "smooth", block: "center" });
    });

    return () => window.cancelAnimationFrame(frame);
  }, [followRecitation, playingVerseKey]);
  ...
  const displayedChapters = useMemo(() => {
    ...
    if (quranBrowseMode === "revelation") {
      items.sort((a, b) => (a.revelationOrder ?? Number.MAX_SAFE_INTEGER) - (b.revelationOrder ?? Number.MAX_SAFE_INTEGER));
    } else {
      items.sort((a, b) => a.id - b.id);
    }
    return quranBrowseAscending ? items : items.reverse();
  }, [filteredChapters, quranBrowseAscending, quranBrowseMode]);
  const displayedJuz = useMemo(() => {
    const query = filter.trim();
    const items = Array.from({ length: 30 }, (_, index) => index + 1).filter((juz) => !query || String(juz).includes(query));
    return quranBrowseAscending ? items : items.reverse();
  }, [filter, quranBrowseAscending]);
  ...
  const changeFollowRecitation = () => {
    setFollowRecitation((current) => {
      const next = !current;
      localStorage.setItem("af-follow-recitation", String(next));
      return next;
    });
  };
  ...
  const renderLibrary = () => <main className={styles.main}>
    <header className={styles.quranDirectoryHeader}><div><p className={styles.kicker}>The Noble Quran</p><h1>{quranBrowseMode === "juz" ? "Choose a Juz" : quranBrowseMode === "revelation" ? "Revelation Order" : "Choose a Surah"}</h1><p>{quranBrowseMode === "juz" ? "Browse all 30 Juz." : quranBrowseMode === "revelation" ? "Browse Surahs by their sourced revelation order." : "Browse all 114 Surahs."}</p></div><nav className={styles.quranBrowseTools} aria-label="Quran reading options"><Link href="/quran/mushaf/1">Mushaf view</Link><Link href="/quran/structure">Structure</Link><Link href="/reflect">Reflections</Link></nav></header>
    <div className={styles.directoryTabsRow}><div className={styles.directoryTabs} role="tablist" aria-label="Browse the Quran"><button type="button" role="tab" aria-selected={quranBrowseMode === "surah"} onClick={() => { setQuranBrowseMode("surah"); setFilter(""); }}>Surah</button><button type="button" role="tab" aria-selected={quranBrowseMode === "juz"} onClick={() => { setQuranBrowseMode("juz"); setFilter(""); }}>Juz</button><button type="button" role="tab" aria-selected={quranBrowseMode === "revelation"} onClick={() => { setQuranBrowseMode("revelation"); setFilter(""); }}>Revelation Order</button></div><button className={styles.sortDirection} type="button" onClick={() => setQuranBrowseAscending(value => !value)} aria-label={`Sort ${quranBrowseAscending ? "descending" : "ascending"}`}>{quranBrowseAscending ? "Ascending ↑" : "Descending ↓"}</button></div>
    <label className={styles.search}><span>⌕</span><input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder={quranBrowseMode === "juz" ? "Search by Juz number" : "Search by Surah name or number"}/><kbd>{quranBrowseMode === "juz" ? "30" : "114"}</kbd></label>
    ...
    {!chaptersLoading && !chaptersError && quranBrowseMode === "juz" ? <div className={styles.juzList}>{displayedJuz.map(juz => <Link href={`/quran/structure/juz/${juz}`} className={styles.juzRow} key={juz}><span className={styles.chapterNumber}>{juz}</span><div><h2>Juz {juz}</h2><p>Read by Quran structure</p></div><b>Read Juz →</b></Link>)}</div> : null}
    {!chaptersLoading && !chaptersError && quranBrowseMode !== "juz" ? <div className={styles.chapterList}>{displayedChapters.map(chapter => <Link href={chapter.readerUrl} className={styles.chapterRow} key={chapter.id}><span className={styles.chapterNumber}>{quranBrowseMode === "revelation" ? chapter.revelationOrder : chapter.id}</span><div><h2>{chapter.nameSimple}</h2><p>{quranBrowseMode === "revelation" ? [`Surah ${chapter.id}`, chapter.revelationPlace, chapter.versesCount ? `${chapter.versesCount} Ayahs` : null].filter(Boolean).join(" · ") : `${chapter.translatedName} · ${chapter.versesCount} Ayahs`}</p></div><strong lang="ar" dir="rtl" translate="no">{chapter.nameArabic}</strong><b>›</b></Link>)}</div> : null}
    {!chaptersLoading && !chaptersError && (quranBrowseMode === "juz" ? displayedJuz.length === 0 : displayedChapters.length === 0) ? <p className={styles.empty}>No {quranBrowseMode === "juz" ? "Juz" : "Surahs"} match your search.</p> : null}
  </main>;
  ...
  const renderReader = () => <main className={`${styles.main} ${styles.readerMain}`} style={{"--arabic-size": `${arabicSize}px`, "--translation-size": `${translationSize}px`} as React.CSSProperties}>
    ...
    {reader ? <>
      ...
      <div className={styles.readerTools}><form onSubmit={jump}><label>Jump to Ayah <input aria-label="Ayah number" inputMode="numeric" name="verse" type="number" min="1" max={reader.chapter.versesCount ?? undefined} defaultValue={Number(verseNumber) || 1}/></label><button>Go</button></form><button className={styles.readerSettingsButton} aria-label="Open reader settings" title="Reader settings" onClick={() => setSettingsOpen(true)}><Icon name="settings"/></button></div>
      <div className={styles.studyTools}><button className={wordMode?styles.selectedStudy:""} onClick={()=>setWordMode(value=>!value)}>Word by word</button><button className={tafsirOpen?styles.selectedStudy:""} onClick={()=>setTafsirOpen(value=>!value)}>Tafsir</button><button aria-expanded={memorizationOpen} className={memorizationOpen?styles.selectedStudy:""} onClick={()=>setMemorizationOpen(value=>!value)}>Memorize</button><button aria-expanded={chapterInfoOpen} className={chapterInfoOpen?styles.selectedStudy:""} onClick={()=>setChapterInfoOpen(value=>!value)}>Introduction</button><Link href="/quran/mushaf/1">Mushaf view</Link><details className={styles.studyMore}><summary>More</summary><div><Link href="/quran/structure">Quran structure</Link><Link href="/reflect">Lessons & reflections</Link></div></details></div>
      {chapterInfoOpen ? <aside className={styles.chapterInfo}>{chapterInfoLoading ? <p>Loading introduction…</p> : null}{chapterInfoError ? <p className={styles.unavailable}>The Surah introduction is unavailable right now.</p> : null}{chapterInfo ? <div translate="no" dangerouslySetInnerHTML={{ __html: chapterInfo.textHtml }} /> : null}</aside> : null}
      ...
      {tajweedEnabled ? <details className={styles.tajweedLegend}><summary>Tajweed color guide</summary><div aria-label="Tajweed color legend"><span><i className={styles.tjMadd}/>Madd</span><span><i className={styles.tjGhunnah}/>Ghunnah</span><span><i className={styles.tjIkhfa}/>Ikhfa</span><span><i className={styles.tjIdgham}/>Idgham</span><span><i className={styles.tjQalqalah}/>Qalqalah</span><span><i className={styles.tjSilent}/>Silent letters</span></div></details> : null}
      <section className={styles.verses} aria-label={`${reader.chapter.nameSimple} verses`}>{reader.verses.map(verse => {const bookmarkId=`quran:${verse.verseKey}`;const isPlaying=playingVerseKey===verse.verseKey&&!audioPaused;const isSaved=hasLocalBookmark(localBookmarks,bookmarkId);return <article id={`verse-${verse.verseNumber}`} data-verse-key={verse.verseKey??undefined} className={`${styles.verse} ${playingVerseKey===verse.verseKey?styles.activeVerse:""}`} aria-current={playingVerseKey===verse.verseKey?"true":undefined} key={verse.id}><div className={styles.verseMeta}><a href={`#verse-${verse.verseNumber}`}>{verse.verseKey}</a><div>{verse.audioUrl ? <button className={styles.playVerse} aria-label={`${isPlaying ? "Pause" : "Play"} recitation of Ayah ${verse.verseNumber}`} onClick={() => playVerse(verse.verseKey)}>{isPlaying ? "❚❚" : "▶"}</button> : null}<button className={styles.saveVerse} aria-label={`${isSaved ? "Remove bookmark from" : "Bookmark"} Ayah ${verse.verseNumber}`} aria-pressed={isSaved} title={isSaved?"Remove bookmark":"Bookmark Ayah"} onClick={()=>setLocalBookmarks(toggleLocalBookmark({id:bookmarkId,label:`${reader.chapter.nameSimple} ${verse.verseKey}`,reference:verse.verseKey??"",type:"quran",url:`/quran/${reader.chapter.id}/${verse.verseNumber}`}))}><Icon name="saved"/></button><details className={styles.verseActions}><summary aria-label={`More actions for Ayah ${verse.verseNumber}`}>•••</summary><div><Link href={`/quran/${reader.chapter.id}/${verse.verseNumber}`}>Open Ayah</Link><button onClick={event=>copyVerseLink(verse.verseKey??`${reader.chapter.id}:${verse.verseNumber}`,reader.chapter.id,verse.verseNumber,event.currentTarget.closest("details"))}>Copy link</button><button onClick={event=>shareVerseLink(verse.verseKey??`${reader.chapter.id}:${verse.verseNumber}`,reader.chapter.id,verse.verseNumber,event.currentTarget.closest("details"))}>Share Ayah</button></div></details>{copiedVerseKey===verse.verseKey?<span className={styles.copyConfirmation} role="status">Copied</span>:null}</div></div>{wordMode&&verse.words.length?<div className={styles.wordGrid} lang="ar" dir="rtl" translate="no">{verse.words.filter(word=>word.charType==="word").map(word=><span className={synchronizedAudio&&playingVerseKey===verse.verseKey&&activeWordPosition===word.position?styles.activeWord:""} key={word.position}><b>{word.arabicText}</b><small dir="ltr">{word.transliteration}</small><em dir="ltr">{word.translation}</em></span>)}</div>:tajweedEnabled && verse.tajweedHtml ? <p className={`${styles.arabic} ${styles.tajweed}`} lang="ar" dir="rtl" translate="no" dangerouslySetInnerHTML={{__html: verse.tajweedHtml}}/> : <p className={styles.arabic} lang="ar" dir="rtl" translate="no">{verse.arabicText}</p>}{verse.translationText ? <><div className={styles.translation} lang="en" translate="no" dangerouslySetInnerHTML={{__html: verse.translationText}}/>{verse.translationName || selectedTranslationResource ? <p className={styles.attribution}>Translation: {verse.translationName ?? selectedTranslationResource?.name}{selectedTranslationResource?.authorName ? ` — ${selectedTranslationResource.authorName}` : ""}</p> : null}{verse.translationFootnotes.length ? <div className={styles.footnotes}>{verse.translationFootnotes.map(footnote=><TranslationFootnote id={footnote.id} label={footnote.label} key={footnote.id}/>)}</div> : null}</> : <p className={styles.unavailable}>The selected translation is unavailable for this Ayah.</p>}{tafsirOpen?<details className={styles.tafsir}><summary>Tafsir · {verse.tafsirName??selectedTafsirResource?.name??"Selected source"}</summary>{verse.tafsirText?<p translate="no">{verse.tafsirText}</p>:<p className={styles.unavailable}>No tafsir entry was returned for this Ayah.</p>}</details>:null}{verse.verseKey ? <AyahStudyReferences verseKey={verse.verseKey} /> : null}</article>})}</section>
      {playingVerseKey ? <div className={styles.audioPlayer}><span aria-live="polite">{synchronizedAudio?"Synchronized recitation":activeRange?`Memorizing ${rangeStart}–${rangeEnd}`:"Reciting"} · Ayah {playingVerseKey}{!synchronizedAudio&&repeatCount>1?` · ${playbackIteration}/${repeatCount}`:""}</span><audio ref={audioRef} controls autoPlay src={synchronizedAudio?.audioUrl??reader.verses.find(verse => verse.verseKey === playingVerseKey)?.audioUrl??undefined} onTimeUpdate={synchronizePlaybackPosition} onEnded={playNextVerse} onPause={()=>setAudioPaused(true)} onPlay={()=>setAudioPaused(false)} onError={stopAudio}>Your browser does not support audio playback.</audio><button className={styles.followPlayback} aria-pressed={followRecitation} onClick={changeFollowRecitation}>{followRecitation?"Following":"Follow"}</button><button aria-label="Close audio player" onClick={stopAudio}>×</button></div> : null}
  ...
  return <div className={styles.app}>
    ...
    {moreOpen?<div className={styles.moreMenu} role="menu" aria-label="More navigation"><Link href="/library" onClick={()=>setMoreOpen(false)}>Saved Library <span>→</span></Link><Link href="/search" onClick={()=>setMoreOpen(false)}>Search <span>→</span></Link>{publicFeatures.qibla ? <Link href="/qibla" onClick={()=>setMoreOpen(false)}>Qibla <span>→</span></Link> : null}{publicFeatures.masjidFinder ? <Link href="/masjid-finder" onClick={()=>setMoreOpen(false)}>Masjid Finder <span>→</span></Link> : null}</div>:null}
    {settingsOpen ? <div className={styles.scrim} onMouseDown={closeSettings}><aside ref={settingsDialogRef} className={styles.settings} role="dialog" aria-modal="true" aria-labelledby="reader-settings" onMouseDown={e => e.stopPropagation()}><header><h2 id="reader-settings">Reader settings</h2><button aria-label="Close settings" onClick={closeSettings}>×</button></header>{reader?<fieldset className={styles.contentSettings}><legend>Quran content</legend><label>Translation<select aria-label="Quran translation" value={selectedTranslation??""} onChange={event=>changeTranslation(Number(event.target.value))}>{translationGroups.map(([language,items])=><optgroup label={language} key={language}>{items.map(item=><option key={item.id} value={item.id}>{item.name}{item.authorName?` — ${item.authorName}`:""}</option>)}</optgroup>)}</select></label><label>Reciter<select aria-label="Quran reciter" value={selectedRecitation??""} onChange={event=>changeRecitation(Number(event.target.value))}>{recitations?.items.map(item=><option key={item.id} value={item.id}>{item.name}{item.style?` — ${item.style}`:""}</option>)}</select></label><label>Arabic script<select aria-label="Quran Arabic script" value={quranScript} onChange={event=>{const value=event.target.value as QuranScript;setQuranScript(value);localStorage.setItem("af-quran-script",value)}}><option value="uthmani">Uthmani</option><option value="uthmani_simple">Uthmani Simple</option><option value="imlaei">Imlaei</option><option value="indopak">IndoPak</option><option value="indopak_nastaleeq">IndoPak Nastaleeq</option></select></label><label>Tafsir source<select aria-label="Quran Tafsir source" value={selectedTafsir??""} onChange={event=>{const value=Number(event.target.value);setSelectedTafsir(value);localStorage.setItem("af-tafsir-id",String(value))}}>{tafsirs?.items.map(item=><option value={item.id} key={item.id}>{item.name}{item.authorName?` — ${item.authorName}`:""}</option>)}</select></label></fieldset>:null}<fieldset><legend>Appearance</legend><div className={styles.themeOptions}>{(["light","dark","sepia"] as Theme[]).map(value=><button className={theme===value?styles.selected:""} onClick={()=>changeTheme(value)} key={value}><i className={styles[value]}/>{value}</button>)}</div></fieldset><label className={styles.toggleSetting}>Tajweed colors <input type="checkbox" checked={tajweedEnabled} onChange={event => { setTajweedEnabled(event.target.checked); localStorage.setItem("af-tajweed", String(event.target.checked)); }}/></label><label>Arabic text size <output>{arabicSize}px</output><input type="range" min="30" max="64" value={arabicSize} onChange={e=>changeSize("arabic", Number(e.target.value))}/></label><label>Translation size <output>{translationSize}px</output><input type="range" min="14" max="26" value={translationSize} onChange={e=>changeSize("translation", Number(e.target.value))}/></label></aside></div> : null}
  </div>;
}
