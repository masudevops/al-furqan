...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={styles.app}>
    ...
    <nav className={styles.mobileNav}><Link className={route === "quran" ? styles.active : ""} href="/quran"><Icon name="quran"/><span>Quran</span></Link><Link className={route === "sunnah" ? styles.active : ""} href="/sunnah"><span className={styles.mobileGlyph}>◉</span><span>Sunnah</span></Link><Link className={route === "salah" ? styles.active : ""} href="/salah-times"><span className={styles.mobileGlyph}>◷</span><span>Salah</span></Link><Link className={route === "dua" ? styles.active : ""} href="/dua"><span className={styles.mobileGlyph}>✦</span><span>Dua</span></Link><button className={route === "search" || route === "qibla" || route === "masjid" ? styles.active : ""} onClick={()=>setMoreOpen(value=>!value)}><Icon name="more"/><span>More</span></button></nav>
    {moreOpen?<div className={styles.moreMenu} role="dialog" aria-label="More navigation"><Link href="/search" onClick={()=>setMoreOpen(false)}>Search <span>→</span></Link><Link href="/qibla" onClick={()=>setMoreOpen(false)}>Qibla <span>→</span></Link><Link href="/masjid-finder" onClick={()=>setMoreOpen(false)}>Masjid Finder <span>→</span></Link></div>:null}
