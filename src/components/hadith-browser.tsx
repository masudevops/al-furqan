...
export default function HadithBrowser({ collectionId, hadithNumber, initialCatalog, initialItem, initialList }: { collectionId?: string; hadithNumber?: string; initialCatalog?: Collection[]; initialItem?: Hadith | null; initialList?: HadithPage }) {
  ...
  if (hadithNumber) return <main className={styles.main}>{detailError ? <ContentState kind="error" message="This Hadith record is unavailable right now. No substitute content has been used." /> : !item ? <ContentState message="Loading Hadith…" /> : <article className={styles.detail}><Link href="/sunnah">← Sunnah library</Link><p className={styles.kicker}>{item.collectionName} · Hadith {item.referenceNumber}</p><h1>{item.sectionName}</h1><p className={styles.arabic} lang="ar" dir="rtl" translate="no">{item.arabic}</p><p className={styles.translation} translate="no">{item.text}</p><section><strong>Reported grade</strong><span>{item.authenticityContext}</span></section><BookmarkButton saved={bookmarked} label={`Hadith ${item.referenceNumber}`} onClick={() => setBookmarks(toggleLocalBookmark({ id: bookmarkId, label: `${item.collectionName} ${item.hadithNumber}`, reference: `${item.collectionName}, Hadith ${item.hadithNumber}`, type: "sunnah", url: `/sunnah/${item.collectionId}/${item.hadithNumber}` }))}/></article>}</main>;
...
function State({ text }: { text: string }) { return <ContentState kind={text.startsWith("Loading") ? "loading" : text.startsWith("No ") ? "empty" : "error"} message={text}/>; }
