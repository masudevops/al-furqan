...
export interface HadithSourceAdapter { collections(): Promise<HadithCollection[]>; list(collection: string, page?: number, query?: string): Promise<HadithListPage>; one(collection: string, number: number): Promise<HadithRecord | null> }
...
const normalizeRecord = (value: unknown): HadithRecord | null => {
  ...
  if (!hadithNumber || !validSlug(collectionId) || !collectionName || !arabic || !english) return null;
...
export const ummahHadithAdapter: HadithSourceAdapter = {
  async collections() {
    ...
    return array(data.collections).map((item) => ({ id: text(item.key), name: text(item.name), sections: [{ id: "all", name: "All available Hadith" }], total: number(item.total_hadiths) })).filter((item) => validSlug(item.id) && item.name && item.total > 0);
  },
